...
export default function LiveToolScreen() {
  ...
  const loadLives = async () => {
    try {
      ...
      const processedLives = data.map((item: any, index: number) => ({
        id: item.id || `live_${index}`,
        title: item.title || `Live Session ${index + 1}`,
      ...
      setLives(processedLives);

      const newSummary = processedLives.reduce(
        (acc: { total: number; stars: number; comments: number; shares: number; views: number }, live: LiveItem) => {
          acc.stars += live.stars;
          acc.comments += live.comments;
          acc.shares += live.shares;
          acc.views += live.views;
          return acc;
        },
        { total: processedLives.length, stars: 0, comments: 0, shares: 0, views: 0 }
      );
  ...
  const renderLiveItem = ({ item }: { item: LiveItem }) => (
    <TouchableOpacity style={styles.liveCard}>
      <View style={styles.liveHeader}>
        <MaterialIcons name="live-tv" size={20} color="#FF0000" />
        ...
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryGrid}>
            <View style={styles.summaryItem}>
              <Text style={styles.summaryLabel}>Total Lives</Text>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <MaterialIcons name="live-tv" size={48} color="#333" />
